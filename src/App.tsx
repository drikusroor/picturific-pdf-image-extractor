...
const Dropzone: React.FC<{ onFiles: (files: FileList) => void }> = ({ onFiles }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-500 mt-1">or click the button below to choose a file</p>
    </div>
  )
}
...
export default function App() {
  ...
  const [meta, setMeta] = useState<{ pages: number, filename?: string } | null>(null)
    ...
    try {
      ...
      for (let pageIndex = 1; pageIndex <= pdf.numPages; pageIndex++) {
        ...
        for (const [id, img] of candidates) {
          try {
            const { width, height, data } = img as { width: number, height: number, data: Uint8ClampedArray }
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <header className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Runs 100% in your browser. No uploads.</p>
      </header>
      ...
      <div className="rounded-2xl bg-white p-5 shadow">
        <div className="flex items-center gap-3">
          ...
          />
          <button onClick={onChoose} className="rounded-xl border bg-gray-900 px-4 py-2 text-white shadow hover:bg-black">
            Choose PDF
          </button>
        ...
        {meta && !busy && (
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              <strong>{meta.filename}</strong> · {meta.pages} page{meta.pages !== 1 ? 's' : ''} · {total} image{total !== 1 ? 's' : ''} found
            </div>
            {images.length > 0 && (
              <button onClick={downloadAll} className="rounded-xl border bg-blue-600 px-4 py-2 text-white shadow hover:bg-blue-700">
                Download all as ZIP
              </button>
            )}
          </div>
      ...
      {images.length > 0 && (
        <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {images.map((img, idx) => (
            <figure key={img.id} className="rounded-xl bg-white p-3 shadow border">
              <img src={img.url} alt={`Extracted ${idx+1}`} className="w-full h-auto rounded-md" />
              <figcaption className="mt-2 text-xs text-gray-600 flex items-center justify-between">
                <span>Page {img.pageIndex} · {img.width}×{img.height}</span>
                <a
                  href={img.url}
      ...
      <footer className="mt-10 text-xs text-gray-500">
        <p>
          Tip: If a PDF only contains vector art or the images were flattened during creation, nothing may be extracted.
          You can still rasterize pages via your PDF tool if needed.
        </p>
      </footer>
    </div>
  )
